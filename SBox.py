...
class SBox:
    def __init__(self):
        self.SBOX_TABLES = self.get_sbox_tables()
    ...
    def get_sbox_substitution(self, xor_x: list, n: int = 4) -> str:
        # S-boxes: substituting the value from s-box table by calculating row and column
        output_sbox = ""
        for j in range(0, 8):
            row = bin2dec(f'{xor_x[j * 6]}{xor_x[j * 6 + 5]}')
            col = bin2dec(f'{xor_x[j * 6 + 1]}{xor_x[j * 6 + 2]}{xor_x[j * 6 + 3]}{xor_x[j * 6 + 4]}')
            val = self.SBOX_TABLES[j][row][col]
            output_sbox += dec2bin(val, n)
